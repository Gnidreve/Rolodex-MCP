...
> `src/channels/telegram.rs` + `telegram_chat_id` in `config.toml`) — der
> "empfohlene erste Schritt" unten ist damit erledigt. ntfy.sh (zweite
> Empfehlung) ist ebenfalls umgesetzt (Branch `ntfy`,
> `src/channels/ntfy.rs`). Der Rest dieses Dokuments ist weiterhin offen.
