...
(E-Mail, Slack, künftig weitere) ein eigenes Tool erzeugt
...
- `config.toml` (per Volume gemountet) = **nur** Kontaktbuch. Welche Felder
  ...
  registrierten Kanäle (aktuell `email` und `slack_webhook_url`):
  ```toml
  [[to]]
  name = "Max Mustermann"
...
- `.env` (per `env_file`) = Zugangsdaten pro Kanal. Für E-Mail: SMTP Host,
  ...
  Pflicht, wenn ihn mindestens ein Kontakt in `config.toml` nutzt. Slack ist
...
- Jeder benötigte Kanal wird beim Start einmal real geprüft
  ...
  Sendeversuch aufzufallen. Ausnahme: Slack hat kein gemeinsames Secret zum
